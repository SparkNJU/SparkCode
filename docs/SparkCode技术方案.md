...
详见 `docs/演示视频脚本.md`。演示任务：修复一个有 bug 的 CLI 计算器（`Calculator/` 目录）。

整体结构：

1. **0:00-0:15** 展示问题：运行 Calculator 测试，25 个测试 13 个失败
2. **0:15-0:30** 启动 Spark Code：欢迎横幅 + 切换模型/模式
3. **0:30-0:40** 输入任务：让 Agent 阅读代码、找出 bug 并修复
4. **0:40-1:30** Agent 自主修复：read 源码 → run 测试 → 定位 3 个 bug → edit 修复 → 测试全部通过
5. **1:30-1:50** 交互特性：Ctrl+O 展开代码块、Tab 补全
6. **1:50-2:00** 退出 Spark Code，手动运行测试确认通过
